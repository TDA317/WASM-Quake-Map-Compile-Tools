...
{
    int numtextures = 0;
    qboolean fSpecific = CheckParm("-contents") || CheckParm("-entities") || CheckParm("-faces") || CheckParm("-models") || CheckParm("-planes") || CheckParm("-textures");
	...
	if (!fSpecific || CheckParm("-planes")) PrintLumpSize(lumpspec, LUMP_PLANES, bsp->numplanes);
	if (!fSpecific || CheckParm("-contents")) PrintLumpSize(lumpspec, LUMP_VERTEXES, bsp->numvertexes);
	if (!fSpecific || CheckParm("-contents")) PrintLumpSize(lumpspec, LUMP_NODES, bsp->numnodes);
	if (!fSpecific || CheckParm("-textures")) PrintLumpSize(lumpspec, LUMP_TEXINFO, bsp->numtexinfo);
	if (!fSpecific || CheckParm("-faces")) PrintLumpSize(lumpspec, LUMP_FACES, bsp->numfaces);
	if (!fSpecific || CheckParm("-contents")) PrintLumpSize(lumpspec, LUMP_CLIPNODES, bsp->numclipnodes);
	if (!fSpecific || CheckParm("-contents")) PrintLumpSize(lumpspec, LUMP_LEAFS, bsp->numleafs);
	if (!fSpecific || CheckParm("-faces")) PrintLumpSize(lumpspec, LUMP_MARKSURFACES, bsp->nummarksurfaces);
	if (!fSpecific || CheckParm("-contents")) PrintLumpSize(lumpspec, LUMP_EDGES, bsp->numedges);
	if (!fSpecific || CheckParm("-faces")) PrintLumpSize(lumpspec, LUMP_SURFEDGES, bsp->numsurfedges);

	if (!fSpecific || CheckParm("-textures")) logprint("%7i %-12s %10i\n", numtextures, "textures", bsp->texdatasize);
	if (!fSpecific || CheckParm("-contents")) logprint("%7s %-12s %10i\n", "", "lightdata", bsp->lightdatasize);
	if (!fSpecific || CheckParm("-contents")) logprint("%7s %-12s %10i\n", "", "visdata", bsp->visdatasize);
	if (!fSpecific || CheckParm("-entities")) logprint("%7s %-12s %10i\n", "", "entdata", bsp->entdatasize);
    }
