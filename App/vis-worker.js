...
async function ensureModule(debug) {
    if (modulePromise && lastDebugFlag === !!debug) {
        return modulePromise;
    }
    lastDebugFlag = !!debug;
    const scriptName = lastDebugFlag ? 'vis-debug.js' : 'vis.js';
    const createFnName = lastDebugFlag ? 'createVisDebugModule' : 'createVisModule';

    try {
        importScripts(scriptName);
    } catch (e) {
        console.warn(`Could not import ${scriptName} in worker:`, e);
        throw e;
    }
    if (typeof self[createFnName] === 'function') {
        modulePromise = self[createFnName]({ print: (t)=>self.postMessage({type:'log', text:String(t)}), printErr: (t)=>self.postMessage({type:'err', text:String(t)}), noInitialRun: true });
        return modulePromise;
    }
    throw new Error(`Module factory '${createFnName}' not found after importing ${scriptName}`);
}
...
self.onmessage = async function(ev) {
    ...
    if (msg.type === 'init') {
        try {
            modulePromise = ensureModule(!!msg.debug);
